...
fn find_sleepiest_guard_minute(spans_by_guard: &HashMap<i32, Vec<SleepSpan>>) -> (i32, i32) {
    let sleepiest = spans_by_guard
        .iter()
        .map(|(k, v)| (k, find_most_slept_minute(&v)))
        .max_by_key(|k| (k.1).1)
        .expect("Expected an answer dude");

    (*sleepiest.0, (sleepiest.1).0)
}

fn find_most_slept_minute(spans: &Vec<SleepSpan>) -> (i32, i32) {
    ...
    let (most_sleepy_minute, times_slept) = histogram.iter().max_by_key(|k| k.1).unwrap();
